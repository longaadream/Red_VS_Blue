...
describe('Demo roster HTTP/WebSocket integration', () => {
  ...
  it('commits exactly one terminal result across a concurrent HTTP and WebSocket surrender', async () => {
    memoryStore.seed(room('terminal-race', 'light'))
    await httpSelect('terminal-race', 'alice', lightRoster)
    await wsSelect('terminal-race', 'bob', lightRoster)
    ...
    try {
      client.send(JSON.stringify({ type: 'subscribe', roomId: 'terminal-race', playerId: 'bob' }))
      ...
      const httpResultPromise = httpBattleAction('terminal-race', 'alice', {
        type: 'surrender',
        playerId: 'alice',
        reason: 'voluntary',
      })
      client.send(JSON.stringify({
        type: 'action',
        action: { type: 'surrender', playerId: 'bob', reason: 'voluntary' },
      }))

      const [httpResult, update] = await Promise.all([httpResultPromise, terminalUpdate])
      expect([200, 409]).toContain(httpResult.status)
      ...
      if (httpResult.status === 200) {
        ...
        expect(httpResult.body).toMatchObject({ code: 'BATTLE_STATE_CONFLICT' })
      }
      ...
      expect(finalStorage.state.terminalResult?.loserPlayerId).toBe(
        httpResult.status === 200 ? 'alice' : 'bob',
      )
