...
async function smokeEditorDistribution(candidate, distribution) {
  ...
  try {
    ...
    const beforeAiEdit = await evaluate(target, `window.editorAPI.readDocument('pieces', 'red178-smoke-piece.json')`)
    const aiFile = path.join(authoringRoot, 'data', 'pieces', 'red178-smoke-piece.json')
    const aiBytes = JSON.stringify({ ...beforeAiEdit.value, description: 'RED-191 external AI edit' }, null, 2) + '\n'
    writeFileSync(aiFile, aiBytes)
    const staleSave = await evaluate(target, `(async () => {
      try {
        await window.editorAPI.writeDocument('pieces', 'red178-smoke-piece.json', ${JSON.stringify(beforeAiEdit.value)}, ${JSON.stringify(beforeAiEdit.revision)})
        return { rejected: false }
      } catch (error) { return { rejected: true, message: error.message } }
    })()`)
    assert(staleSave.rejected && staleSave.message.includes('文件已被 AI'), 'Packaged Editor accepted a stale AI overwrite')
    assert(readFileSync(aiFile, 'utf8') === aiBytes, 'Packaged Editor changed external AI bytes after rejecting save')
    const projectControls = await evaluate(target, `({
      modes: [...document.querySelectorAll('[data-project]')].map(button => button.dataset.project),
      refresh: Boolean(document.getElementById('project-refresh')),
      reveal: Boolean(document.getElementById('project-reveal')),
    })`)
    assert(['official', 'blank', 'open'].every(mode => projectControls.modes.includes(mode)) && projectControls.refresh && projectControls.reveal, 'Packaged Editor project controls are missing')
