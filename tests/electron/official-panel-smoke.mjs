...
const root = process.cwd(), built = path.join(root, 'dist/official-server', process.env.RVB_OFFICIAL_BUILD_VARIANT || 'win-x64'), output = path.join(root, 'dist/multiplayer-qa')
...
try {
  ...
  await until(() => page.evaluate('document.getElementById("capacity-input").value === "10"'), 'capacity form')
  await page.evaluate('document.getElementById("capacity-input").value="3";document.getElementById("capacity-form").requestSubmit();document.getElementById("confirm-reason").value="浏览器容量测试";document.getElementById("confirm-ok").click()')
  await until(() => page.evaluate('document.getElementById("capacity").textContent.includes("3 局") && !document.getElementById("refresh").disabled'), 'capacity persisted')
  await page.evaluate('document.getElementById("announcement-input").value="试玩公告 <script>无脚本执行</script>";document.getElementById("announcement-input").dispatchEvent(new Event("input"));document.getElementById("announcement-form").requestSubmit();document.getElementById("confirm-reason").value="公告测试";document.getElementById("confirm-ok").click()')
  await until(async () => (await fetch(`http://127.0.0.1:${port}/official/info`).then(r => r.json())).announcement?.includes('试玩公告'), 'public announcement')
  await until(() => page.evaluate('!document.getElementById("refresh").disabled'), 'announcement complete')
  await page.evaluate('document.querySelector("[data-view=backups]").click()')
  await until(() => page.evaluate('document.getElementById("backup-rows").textContent.includes("暂无记录")'), 'empty backups')
  await page.evaluate('document.getElementById("backup-create").click();document.getElementById("confirm-reason").value="恢复演练";document.getElementById("confirm-ok").click()')
  await until(() => page.evaluate('document.getElementById("backup-rows").textContent.includes("MiB") && !document.getElementById("refresh").disabled'), 'actual PG backup')
  const backupId = await page.evaluate('api("config").then(c=>c.backups[0].id)')
  await page.evaluate('perform("capacity","7",{reason:"恢复前改变容量"})')
  await until(() => page.evaluate('document.getElementById("capacity").textContent.includes("7 局")'), 'capacity changed before restore')
  await page.evaluate(`confirmAction('backup-restore',${JSON.stringify(backupId)},'浏览器恢复演练');document.getElementById('confirm-reason').value='恢复演练';document.getElementById('confirm-ok').click()`)
  await until(() => page.evaluate('document.getElementById("capacity").textContent.includes("3 局") && !document.getElementById("refresh").disabled'), 'actual PG restored')
  if (!await page.evaluate('snapshot.settings.maintenance === true')) throw new Error('Restore reopened matchmaking')
  fs.writeFileSync(path.join(output, 'red196-control-panel-management.png'), Buffer.from((await page.call('Page.captureScreenshot', { format: 'png' })).data, 'base64'))
