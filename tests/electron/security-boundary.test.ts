...
describe('Electron desktop security boundary', () => {
  ...
  test('runs an isolated real renderer smoke before committing stable', () => {
    ...
    expect(source).toContain('PROFILE_RENDERER_RESOURCE_FETCH_FAILED')
    expect(source).not.toContain("serverLiteral} + '/api/ping'")
    const healthStage = source.indexOf("stage = 'candidate-server-health'")
    const websocketStage = source.indexOf("stage = 'candidate-websocket-health'")
    const rendererStage = source.indexOf("stage = 'candidate-renderer-preflight'")
    expect(healthStage).toBeLessThan(websocketStage)
    expect(websocketStage).toBeLessThan(rendererStage)
