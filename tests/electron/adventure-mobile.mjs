...
try{
  ...
  await evaluate(`(()=>{const original=battlePresentation.animateAction.bind(battlePresentation);window.__qaMoveAnimations=0;battlePresentation.animateAction=function(action,...rest){if(action.type==='move')window.__qaMoveAnimations++;return original(action,...rest)}})()`)
  ...
  const moveElapsedMs=Math.round(performance.now()-moveStarted)
  const moveAnimations=await evaluate('window.__qaMoveAnimations')
  assert.equal(moveAnimations,1,'Committed move must animate once, including duplicate broadcast/reply delivery')
  ...
  console.log(JSON.stringify({apkWebView:true,lobbyViewports:[[393,851],[640,360],[800,360]],battleTouchViewport:actualSize,cellPixels:tactical.spacing,touchMove:true,touchDetails:true,manualSaves:saves.length,moveAnimations,moveElapsedMs}))
}finally{ws.close()}
