...
function stateWith(pieces: any[]) {
  const state = makeState({ pieces, currentPlayerId: 'blue1' }) as any
...
it('holy cards target teammates as allies and never smite a low-health teammate', () => {
  ...
  expect(ally.statusTags.some((s: any) => s.type === 'damage-buff')).toBe(true)
  expect(enemy.statusTags.some((s: any) => s.type === 'damage-buff')).toBe(false)
})
it.each(['skills-loader', 'rule-loader'])('blood echo gives one card per allied Liadrin owner through %s trigger contexts', loader => {
  ...
  new TriggerSystem().checkTriggers(state, { type: 'afterStatusRemoved', playerId: 'blue2', sourcePiece: target, statusId: 'divine-shield' } as any)
  expect(state.players.map((p: any) => p.hand.filter((c: any) => c.cardId === 'holy-charge').length)).toEqual([1, 0, 0, 1])
})
it('two Tracers count each enemy action exactly once per instance, including restored serialized state', () => {
  ...
  new TriggerSystem().checkTriggers(state, { type: 'afterMove', playerId: 'red1', sourcePiece: enemy } as any)
  expect(state.extensions.recallData.map((r: any) => r.actionCount)).toEqual([1, 1])
  const restored = JSON.parse(JSON.stringify(state))
  for (const p of restored.pieces.filter((p: any) => p.templateId === 'tracer')) p.rules = [loadRuleById('rule-recall-move', true)!]
  new TriggerSystem().checkTriggers(restored, { type: 'afterMove', playerId: 'red1', sourcePiece: restored.pieces[2] } as any)
  expect(restored.extensions.recallData.map((r: any) => r.actionCount)).toEqual([2, 2])
})
it('blizzard waits past a teammate turn without consuming its status', () => {
  ...
  const result = executeSkillFunction(definition('skills', 'blizzard-damage'), { piece: caster, player, playerId: 'red1', battle: state, skill: {} } as any, state)
...
it('two Shishio casters apply each permanent burning tile only once', () => {
  ...
  new TriggerSystem().checkTriggers(state, { type: 'afterMove', playerId: 'blue1', sourcePiece: mover } as any)
...
it('two Sasuke player rules stack and damage once while preserving the tile source', () => {
  ...
  new TriggerSystem().checkTriggers(state, { type: 'endTurn', playerId: 'blue1' } as any)
