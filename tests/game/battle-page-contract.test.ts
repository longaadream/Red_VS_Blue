...
describe('battle page route contract', () => {
  ...
  it('keeps target submission single-flight and clears transient targeting on every authoritative exit', () => {
    ...
    expect(battlePage).toContain('function reconcileBattleInteractionState(previousState, nextState, acceptedClientActionId)')
    ...
    expect(battlePage).toMatch(/if \(rejectedPending\)[\s\S]*?rejectPendingActionFeedbackForClientAction\([\s\S]*?preserveTargetInteraction: true/)
