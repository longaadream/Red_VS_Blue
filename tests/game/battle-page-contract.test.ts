...
describe('battle page route contract', () => {
  ...
  it('keeps one responsive HUD, board-anchored piece menu, and curved hand structure', () => {
    ...
    expect(battlePage).toContain('class="hand-label"')
    expect(contextCss).toMatch(/\.hand-panel\s*\{[\s\S]*?border:\s*0\s*!important;[\s\S]*?background:\s*transparent\s*!important;/)
    expect(contextCss).toMatch(/\.training-bar\s*\{[\s\S]*?border:\s*0\s*!important;[\s\S]*?background:\s*transparent\s*!important;[\s\S]*?backdrop-filter:\s*none;/)
    expect(contextCss).toMatch(/\.hand-panel::after\s*\{\s*content:\s*none\s*!important;\s*\}/)
    expect(battlePage).not.toContain('color:#64748b;border-bottom:1px solid #1a2540')
