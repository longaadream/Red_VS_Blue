...
describe('battle page route contract', () => {
  ...
  it('feeds the authoritative response timer into the shared battle clock view', () => {
    ...
    expect(battlePage).toContain("authoritativePendingTimer ? '响应 ' + view.clockText : '回合 ' + view.clockText")
    expect(battlePage).toContain("clock.parentElement?.classList.toggle('pending-timer-active', !!authoritativePendingTimer)")
    expect(battlePage).toContain('class="turn-clock-dock"')
  ...
  it('routes training through local authority and tutorial directly to its local battle mode', () => {
    ...
    expect(lobby).toContain("function goToTutorial() { window.location.href = 'tutorial.html' }")
...
new Script([
  ...
  it('shows authoritative reserve-candidate stats and opens read-only accessible details', () => {
    ...
    expect(battlePage).toContain('id="pieceInfoIdentity"')
    expect(battlePage).toContain("badge.textContent = isCore ? '核心棋子' : '非核心棋子'")
    expect(battlePage).toContain("piece.isCore === true")
  ...
  it('keeps player status icons above auxiliary HUD panels and makes the turn clock explicit', () => {
    const battlePage = readPage('battle.html')
    const contextUi = readPage('css/battle-context-ui.css')

    expect(contextUi).toContain('body #playerResCards { overflow:visible !important; }')
    expect(contextUi).toMatch(/\.phase-bar:has\(\.player-effect-icon\)[^{]*\{[^}]*z-index:96 !important/)
    expect(contextUi).toMatch(/body #playerResCards \.player-state-tags[\s\S]*?position:absolute;[\s\S]*?z-index:4;/)
    expect(battlePage).toContain("'回合 ' + view.clockText")
    expect(battlePage).toMatch(/\.turn-clock-dock \{[\s\S]*?position: fixed; top: 88px; right: 22px; z-index: 95/)
    expect(battlePage).toMatch(/\.turn-clock \{[\s\S]*?background: #f0d494;[\s\S]*?font-size: 17px/)
    expect(battlePage).toMatch(/\.turn-clock\.burning \{[\s\S]*?background: #a33d2f/)
  })
})
