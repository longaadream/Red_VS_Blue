...
describe('battle piece health and negative-status summary', () => {
  ...
  it('wires the shared presentation config into Three.js, fallback tokens, and the full detail modal', () => {
    ...
    expect(renderer).toContain('loadGeneration !== _textureLoadGeneration')
    expect(renderer).toContain('_textureLoadGeneration += 1')
    expect(renderer).toMatch(/_texCache\.forEach\(function \(entry\)[\s\S]*?entry\.texture\.dispose/)
