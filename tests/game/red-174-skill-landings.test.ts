...
import type { PieceSkill } from '@/lib/game/piece'
...
function installSkill(state: BattleState, piece: { skills: PieceSkill[] }, skillId: string): void {
  state.skillsById[skillId] = loadAllSkillsById()[skillId]
  piece.skills = [{ skillId, currentCooldown: 0, usesRemaining: -1 }]
}

function expectUniqueLivingCells(state: BattleState): void {
  const cells = state.pieces
    .filter((piece) => piece.currentHp > 0)
...
describe('RED-174 skill landing contracts', () => {
  it('lets Illidan Blade Dash damage through enemies but land only on an open cell', () => {
    ...
    const state = makeState({ pieces: [illidan, enemy], width: 7, height: 3 })
    installSkill(state, illidan, 'illidan-blade-dash')

    const prepared = prepareAction(state, {
      type: 'useBasicSkill' as const, playerId: 'player-red', pieceId: illidan.instanceId,
    ...
    expect(resolved.pieces.find((piece) => piece.instanceId === illidan.instanceId)).toMatchObject({ x: 3, y: 1 })
    expect(resolved.pieces.find((piece) => piece.instanceId === enemy.instanceId)!.currentHp).toBeLessThan(enemy.currentHp)
  ...
  it('keeps Grimmjow hunting destinations away from cells reserved by the triggering skill', () => {
    ...
    state.skillsById['reserved-cell-skill'] = {
      range: 'single', id: 'reserved-cell-skill', name: '预留落点技能', description: '', kind: 'active', type: 'normal',
    ...
    const base = {
      type: 'useBasicSkill' as const, playerId: 'player-blue', pieceId: enemy.instanceId, skillId: 'reserved-cell-skill',
    }
  ...
  it('reserves Aizen Shunpo landing so Flying Raijin cannot take it during beforeSkillUse', () => {
    ...
    const state = makeState({ pieces: [aizen, minato, target, ...blockers], width: 6, height: 5 })
    installSkill(state, aizen, 'aizen-shunpo')
    state.players[0].actionPoints = 2
    const base = {
      type: 'useBasicSkill' as const, playerId: 'player-red', pieceId: aizen.instanceId, skillId: 'aizen-shunpo',
    }
    const prepared = prepareAction(state, base)
    expect(prepared.kind).toBe('needTarget')
    if (prepared.kind !== 'needTarget') return
    ...
    const pendingRun = runBattleAction(state, action, { rootSeed: 174 })
    const repeatedPendingRun = runBattleAction(state, action, { rootSeed: 174 })
    expect(repeatedPendingRun.stateHash).toBe(pendingRun.stateHash)
    const pending = pendingRun.state
    expect(pending.pendingOptionSelection).toMatchObject({ playerId: 'player-red' })
    const resolveAction = {
      type: 'pendingOptionSelect' as const, playerId: 'player-red', selectedOption: 'yes',
      selectionId: pending.pendingOptionSelection!.selectionId,
      stateRevision: pending.pendingOptionSelection!.stateRevision,
    }
    const resolvedRun = runBattleAction(pending, resolveAction, { rootSeed: 174 })
    const repeatedResolvedRun = runBattleAction(repeatedPendingRun.state, resolveAction, { rootSeed: 174 })
    expect(repeatedResolvedRun.stateHash).toBe(resolvedRun.stateHash)
    const resolved = resolvedRun.state

    expect(resolved.pendingOptionSelection).toBeUndefined()
    expect(resolved.pieces.find((piece) => piece.instanceId === aizen.instanceId)).toMatchObject({ x: 3, y: 1 })
    expect(resolved.pieces.find((piece) => piece.instanceId === minato.instanceId)).toMatchObject({ x: 4, y: 2, attack: 4 })
    expectUniqueLivingCells(resolved)
  })
  ...
  it('cancels Shadow Step exact teleport and cleans its marker when the saved cell is occupied', () => {
    ...
    reaper.name = '死神'
    const blocker = makePiece({ instanceId: 'shadow-step-blocker', ownerPlayerId: 'player-blue', x: 2, y: 1 })
    const state = makeState({ pieces: [reaper, blocker], width: 4, height: 3 })
  ...
  it('offers Kenshin only enemies that currently have a legal nearby landing', () => {
    const kenshin = makePiece({ instanceId: 'kenshin', ownerPlayerId: 'player-red', x: 0, y: 0, attack: 4 })
    const blocked = makePiece({ instanceId: 'blocked-enemy', ownerPlayerId: 'player-blue', x: 2, y: 2 })
    const open = makePiece({ instanceId: 'open-enemy', ownerPlayerId: 'player-blue', x: 5, y: 2 })
    const blockers = [[2, 1], [1, 2], [3, 2], [2, 3]].map(([x, y], index) => makePiece({
      instanceId: `kenshin-blocker-${index}`, ownerPlayerId: 'player-red', x, y,
    }))
    const state = makeState({ pieces: [kenshin, blocked, open, ...blockers], width: 7, height: 5 })
