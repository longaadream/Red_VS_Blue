...
describe('six hands-on lessons', () => {
  it('does not offer the disabled sixth lesson after winning lesson five', async () => {
    const f = await fixture('charge')
    f.state().terminalResult = { winnerPlayerId: f.lesson.player.playerId, reason: 'core-eliminated' }
    f.runtime.showResult()
    expect(f.labels()).toContain('返回课程')
    expect(f.labels()).not.toContain('下一局')
  })
  ...
  it('charge can generate its crystal through a real kill, collect it and use a charge skill', async () => {
    const f = await fixture('charge')
    await f.click('开始学习')
    await f.legal(a => a.type === 'deployReservePiece')
    await f.click('学习本回合首移')
    ...
    for (let n = 0; n < 30 && f.runtime.snapshot().openingStep === 'fight-crystal' && !f.state().terminalResult; n++) {
      ...
      const action = plan?.actions.map(draft => prepareLegalBotAction(f.state(), draft, f.lesson.player.playerId)).find(Boolean)
      expect(action).toBeDefined()
      route.push(action)
      await f.act(action)
    }
    expect(f.state().terminalResult, JSON.stringify(route)).toBeUndefined()
    expect(f.runtime.snapshot().openingStep, JSON.stringify(route)).toBe('collect')
    const crystal = f.state().extensions.tileEffects.find(t => t.tileType === 'charge-crystal')
    for (let n = 0; n < 8 && f.runtime.snapshot().openingStep === 'collect'; n++) {
      const legal = listLegalAIActions(f.state(), f.lesson.player.playerId).map(c => c.action)
      const pickup = legal.find(a => a.type === 'move' && a.toX === crystal.x && a.toY === crystal.y)
