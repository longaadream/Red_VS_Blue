...
describe('historical main board', () => {
  ...
  it('finishes successful training placement before rendering, preserving failed placement for retry', async () => {
    ...
    for (const failed of [false, true]) {
      ...
      const context = { ADVENTURE_MODE: false, PRACTICE_MODE: false, togglePlaceMode: () => {}, sendPatch: async (body: unknown) => { void body; return false }, G: old, placingMode: true, trainingApiFetch: async () => { if (failed) throw Error('rejected'); return next },
