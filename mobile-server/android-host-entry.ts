...
async function main() {
  ...
  const { server, journal } = createColyseusBattleServer({ repository, adventureRepository: repository.adventureRepository(), requireIdentityProof: true, healthIdentity: { runtime: 'colyseus-android', database: 'sqlite' } })
