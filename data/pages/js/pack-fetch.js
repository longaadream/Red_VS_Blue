...
;(function () {
    ...
    try {
      ...
      if (file && file.data) {
        try {
          ...
          console.warn('[pack-fetch] JSON parse error for', path, e)
        }
      }
      var res = await nativeFetch(path)
      if (!res.ok) {
        console.warn('[pack-fetch] HTTP', res.status, 'for', path)
        return null
      }
      try { return await res.json() } catch (e) { console.warn('[pack-fetch] JSON parse error (network) for', path, e); return null }
    } catch (e) {
      console.warn('[pack-fetch] fetchPackJson failed for', path, e)
      return null
    }
  }
  ...
  window.fetchPackHtml = async function fetchPackHtml (pageName) {
    try {
      ...
      if (!res.ok) { console.warn('[pack-fetch] HTTP', res.status, 'for', pageName); return null }
      return res.text()
    } catch (e) {
      console.warn('[pack-fetch] fetchPackHtml failed for', pageName, e)
      return null
    }
  }
