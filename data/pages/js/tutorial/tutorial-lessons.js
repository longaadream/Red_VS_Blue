(function (global) {
  ...
  const lessons = [
    {
      id: 'first-victory', number: 1, title: '第一场胜利', size: 2, opponentSize: 1, opponentHp: 3, ownTurn: 3,
      ...
      intro: '消灭敌方场上全部核心即可获胜。先学习移动和攻击。',
      help: '先点乌瑟尔看“祝福之锤”：1 AP、两格内一个敌人。可以移动后再使用，也可以让吉安娜先行动。保护自己的场上核心。',
      placement: [[PLAYER, 'uther', 6, 7], [PLAYER, 'jaina', 5, 8], [OPPONENT, 'reaper', 10, 7]],
      cue: { cells: [{ x: 8, y: 7 }] },
    },
    {
      id: 'reinforcements', number: 2, title: '安排增援与行动', size: 3, opponentSize: 2, opponentHp: 4, ownTurn: 1, progressive: true,
      guidedOpening: { kind: 'deployment' },
      summary: '自己选择增援和落点，安排每回合的行动点与手牌。',
      intro: '选择增援、部署到高亮格，再安排本回合行动。',
      help: '候选中选一枚，再点合法格完成免费部署。新棋子本回合第一次普通移动免费，也可以留在原位行动。幸运币点一次预览，再点同一张打出。',
      placement: [[PLAYER, 'uther', 6, 7], [OPPONENT, 'reaper', 12, 7]],
    },
    {
      id: 'protect-cores', number: 3, title: '保护自己的核心', size: 3, opponentSize: 2, opponentHp: 4, ownTurn: 4,
      ...
      intro: '乌瑟尔只剩 8 点生命，先治疗，再用圣盾保护他。',
      help: '安度因的圣光闪耀花 1 AP，治疗七格内友军。圣光盾要先付 2 AP，对自己施放返还 1 AP。',
      placement: [[PLAYER, 'uther', 6, 7, 8], [PLAYER, 'jaina', 5, 8], [PLAYER, 'anduin', 6, 6], [OPPONENT, 'reaper', 9, 7], [OPPONENT, 'red-blackwidow', 14, 7]],
    },
    {
      id: 'terrain', number: 4, title: '找到进攻机会', size: 4, opponentSize: 2, opponentHp: 4, ownTurn: 4,
      ...
      intro: '利用掩体调整站位，寻找进攻机会。',
      help: '掩体可以站人，但挡弹射物；墙不能站且挡弹道；洞穴不能站但允许弹道通过。按技能范围选择目标。',
      placement: [[PLAYER, 'uther', 13, 8], [PLAYER, 'jaina', 12, 7], [PLAYER, 'anduin', 11, 9], [PLAYER, 'tracer', 8, 7], [OPPONENT, 'red-blackwidow', 17, 8], [OPPONENT, 'reaper', 16, 7]],
      cue: { cells: [{ x: 15, y: 8 }] },
    },
    {
      id: 'charge', number: 5, title: '把优势变成胜利', size: 3, opponentSize: 2, opponentHp: 3, ownTurn: 1, progressive: true,
      guidedOpening: { kind: 'charge' },
      summary: '判断结晶争夺风险，决定何时使用队伍充能。',
      intro: '击败核心后争夺结晶，用充能技能扩大优势。',
      help: '核心阵亡留下中立结晶。普通移动到结晶格可拾取，双方都能争夺。充能技能同时检查 AP、CP 和冷却，先看费用再决定何时使用。',
      placement: [[PLAYER, 'uther', 6, 7], [OPPONENT, 'reaper', 12, 7]],
    },
    {
      id: 'full-match', number: 6, title: '独立综合对战', enabled: false, size: 8, opponentSize: 4, opponentHp: 6, ownTurn: 1, progressive: true,
      guidedOpening: { kind: 'full-match' },
      summary: '基础 AI 综合试玩：八枚阵容对四枚残血敌人，练习部署与收官。',
      intro: '综合运用部署、移动和技能，消灭敌方场上核心。',
  ...
  async function createBattle(engine, lesson) {
    ...
    state.extensions.tutorialLesson = { id: lesson.id, rootSeed: lesson.rootSeed, scenario: true }
    return state
  }
  ...
  function observe(lesson, before, after, action, seen) {
    ...
    if (crystals.some(function (t) { return t.tileType === 'charge-crystal' })) once('crystal', '阵亡位置留下了中立充能结晶，双方都可以争夺。普通移动落在结晶格可拾取。')
    ...
    if (cores.length === 1) once('last-core', '这是你最后一枚场上核心。保护好它。')
    after.pieces.forEach(function (p) {
      const old = before.pieces.find(function (candidate) { return candidate.instanceId === p.instanceId })
      if (p.ownerPlayerId === PLAYER && old && p.currentHp > old.currentHp) once('healed', p.name + '恢复了 ' + (p.currentHp - old.currentHp) + ' 点生命。')
    })
    return notices
  }
