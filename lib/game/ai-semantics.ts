...
export function stableAiHash(value: unknown): string {
  const canonicalize = (input: unknown): unknown => {
    ...
    if (input && typeof input === 'object') return Object.fromEntries(Object.keys(input).sort().map(key => [key, canonicalize((input as Record<string, unknown>)[key])]))
    return input
  }
...
function visibleStatuses(piece: SemanticPiece): AiStatusFeature[] {
  return (piece.statusTags || []).filter(tag => tag && tag.visible !== false).map(tag => ({
...
export function transitionFeatures(before: BattleLike, after: BattleLike): AiTransitionFeatures {
  ...
  for (const [id, next] of newPieces) {
    ...
    const oldTags = new Set((previous.statusTags || []).map(tag => tag.id || tag.type))
    const nextTags = new Set((next.statusTags || []).map(tag => tag.id || tag.type))
    statusAdded += [...nextTags].filter(tag => !oldTags.has(tag)).length
    statusRemoved += [...oldTags].filter(tag => !nextTags.has(tag)).length
  }
  const resourceDelta = (after.players || []).reduce((total, player) => total + Number(player.actionPoints || 0) + Number(player.chargePoints || 0), 0)
    - (before.players || []).reduce((total, player) => total + Number(player.actionPoints || 0) + Number(player.chargePoints || 0), 0)
...
export function candidateActionFeatures(action: SemanticAction, metadata: { mechanics?: AiMechanic[]; compatibility?: AiCompatibility; diagnostics?: string } = {}): AiCandidateActionFeatures {
