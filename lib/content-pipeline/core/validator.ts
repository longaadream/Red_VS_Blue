...
import { createReadonlyContentTreeV1 } from './source'
import type {
  ContentPackSourceV1,
...
function cloneBytes(value: unknown, stage: ContentPipelineStageV1): Uint8Array {
  if (!(value instanceof Uint8Array)) reject('PACK_SCHEMA_INVALID', stage)
...
function preflightSource(source: ContentPackSourceV1): PreflightedSourceV1 {
  if (source === null || typeof source !== 'object' || !Array.isArray(source.entries)) {
  ...
  if (
    source.manifestBytes.byteLength === 0
    || source.manifestBytes.byteLength > CONTENT_PIPELINE_LIMITS_V1.maxManifestBytes
  ) {
    reject('PACK_BUDGET_EXCEEDED', 'source')
  }

  const orderedEntries = [...source.entries]
    .map(entry => ({ entry, path: rawEntryPath(entry) }))
    .sort((left, right) => (
      compareUnicodeCodePointsV1(left.path, right.path)
    ))
  if (orderedEntries.length > CONTENT_PIPELINE_LIMITS_V1.maxEntries) {
    reject('PACK_BUDGET_EXCEEDED', 'source')
  }
  ...
  for (const ordered of orderedEntries) {
    ...
    seenPaths.add(key)
    const rawBytes: unknown = entry.bytes
    if (!(rawBytes instanceof Uint8Array)) {
      reject('PACK_SCHEMA_INVALID', 'source', { path })
    }
    const bytes = new Uint8Array(rawBytes)
    if (bytes.byteLength > CONTENT_PIPELINE_LIMITS_V1.maxFileBytes) {
  ...
  if (source.signatureBytes !== undefined && source.signatureBytes !== null) {
    if (!(source.signatureBytes instanceof Uint8Array)) {
      reject('PACK_SCHEMA_INVALID', 'source')
    }
    if (
      source.signatureBytes.byteLength === 0
      || source.signatureBytes.byteLength > CONTENT_PIPELINE_LIMITS_V1.maxSignatureBytes
    ) {
      reject('PACK_BUDGET_EXCEEDED', 'source')
    }
    signatureBytes = new Uint8Array(source.signatureBytes)
  }

  return {
    manifestBytes: new Uint8Array(source.manifestBytes),
    signatureBytes,
...
function validateSignature(
  ...
  try {
    ...
    return reject('PACK_SIGNATURE_INVALID', 'signature', { packId: manifest.packageId })
  }
...
function factFromTree(
  tree: ReadonlyContentTreeV1,
  descriptor: PackFileDescriptorV1,
  packId: string,
): ValidatedFileFactV1 {
  const bytes = tree.readFile(descriptor.path)
  if (bytes === undefined) {
    reject('PACK_FILE_MISSING', 'patch', { packId, path: descriptor.path })
  }
  return validateFileContent(descriptor, bytes, true, packId)
}
...
function derivePatchCapabilities(
  ...
  parent: ReadonlyContentTreeV1,
  policy: ContentValidationPolicyV1,
): readonly PackCapabilityV1[] {
  const factsByPath = new Map(facts.map(fact => [fact.descriptor.path, fact]))
  const parentByPath = new Map(parent.files.map(file => [file.path, file]))
  const parentByCollision = new Map(parent.files.map(file => [collisionKey(file.path), file]))
  ...
  for (const operation of manifest.operations) {
    ...
    if (operation.op === 'add') {
      ...
      if (exactParent === undefined || exactParent.sha256 !== operation.expectedHash) {
        reject('PATCH_PRECONDITION_FAILED', 'patch', {
          packId: manifest.packageId,
          path: operation.targetPath,
        })
      ...
      if (
        ...
        && parent.hasExecutableContent(operation.targetPath)
      ) {
      ...
      addFactCapabilities(
        capabilities,
        factFromTree(parent, exactParent, manifest.packageId),
      )
    }
...
export function validatePackSourceV1(
  ...
  assertCompatibility(
    ...
    policy.expectedCompatibility,
    manifest.packageId,
  )
  if (policy.kind === 'bundled-base' && manifest.kind !== 'snapshot') {
  ...
  if (manifest.kind === 'patch') {
    if (context.parent === undefined) {
      reject('PACK_REFERENCE_INVALID', 'patch', { packId: manifest.packageId })
    }
    capabilities = derivePatchCapabilities(manifest, facts, context.parent, policy)
  } else {
  ...
  return Object.freeze({
    ...
    networkEligible: policy.kind !== 'local-dev',
    capabilities,
  })
}

function validateCandidateFiles(
  files: readonly ResolvedCandidateFileInputV1[],
  trustedExecutablePaths: readonly string[],
): ValidatedFileFactV1[] {
  if (files.length > CONTENT_PIPELINE_LIMITS_V1.maxEntries) {
    reject('PACK_BUDGET_EXCEEDED', 'source')
  }
  const trusted = new Set(trustedExecutablePaths)
  if (trusted.size !== trustedExecutablePaths.length) {
    reject('PACK_CAPABILITY_MISMATCH', 'capability')
  }
  const seen = new Set<string>()
  const facts: ValidatedFileFactV1[] = []
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    const parsedDescriptor = PackFileDescriptorV1Schema.safeParse(input.descriptor)
    if (!parsedDescriptor.success) reject('PACK_SCHEMA_INVALID', 'profile')
    const descriptor = parsedDescriptor.data
    if (
      index > 0
      && compareUnicodeCodePointsV1(files[index - 1].descriptor.path, descriptor.path) > 0
    ) {
      reject('PACK_SCHEMA_INVALID', 'profile', { path: descriptor.path })
    }
    const key = collisionKey(descriptor.path)
    if (seen.has(key)) reject('PACK_PATH_COLLISION', 'profile', { path: descriptor.path })
    seen.add(key)
    const bytes = cloneBytes(input.bytes, 'profile')
    if (bytes.byteLength > CONTENT_PIPELINE_LIMITS_V1.maxFileBytes) {
...
export function validateResolvedCandidateV1(
  ...
  assertCompatibility(input.compatibility, expectedCompatibility)
  const facts = validateCandidateFiles(input.files, input.trustedExecutablePaths)
