...
const names = { 'map-pool': '修改排位地图池', maintenance: '匹配维护', ban: '封禁账号', unban: '解封账号', season: '切换赛季', stop: '停止服务', kick: '踢下线', 'rank-disable': '限制排位资格', 'rank-enable': '恢复排位资格', 'cooldown-clear': '解除匹配冷却', 'queue-clear': '清空匹配队列', capacity: '修改并发上限', announcement: '更新公告', 'void-match': '作废异常对局', 'smtp-save': '修改SMTP', 'backup-create': '创建备份', 'backup-check': '校验备份', 'backup-restore': '恢复备份' };
...
async function loadView(ticket) {
  if (view === 'settings') {
    if (!$('map-pool-form').dataset.edited) document.querySelectorAll('#map-pool-options input').forEach(input => { input.checked = snapshot.settings.ranked_maps.includes(input.value); });
...
$('map-pool-form').onchange = () => { $('map-pool-form').dataset.edited = 'true'; };
$('map-pool-form').onsubmit = e => { e.preventDefault(); const maps = Array.from(document.querySelectorAll('#map-pool-options input:checked'), input => input.value); if (maps.length < 3) { notice('至少保留3张地图。'); return; } void confirmAction('map-pool', JSON.stringify(maps), '保存地图池，仅对之后匹配的新对局生效。'); };
