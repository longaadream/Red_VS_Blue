...
import { RoguelikeAdventureV1Schema, RoguelikeSuppliesV1Schema } from '../contracts/roguelike-content-v1'
...
export function loadAdventureContent(read = (relative: string) => JSON.parse(readFileSync(join(getDataRoot(), relative), 'utf8'))) {
  return RoguelikeAdventureV1Schema.parse(read('pve/roguelike/adventure.json'))
}
