import { resolveZeroStageConfig } from './evaluator-config'
import type {
  AIObservation,
  AIObservedPiece,
  ZeroStageConfig,
...
function lethalOpportunities(sources: AIObservedPiece[], targets: AIObservedPiece[]) {
  return targets.reduce((total, target) => {
    const threatened = sources.some(source => (
      ...
      // This is ordinary attack potential, not a prediction of arbitrary skill
      // code. Public shield must be depleted before health can be finished.
      && Math.max(1, source.attack - target.defense) >= target.currentHp + Math.max(0, target.shield ?? 0)
    ))
...
function enemyProximityPotential(
  ...
  const proximity = (source: AIObservedPiece, target: AIObservedPiece) => {
    ...
    if (!Number.isFinite(steps)) return 0
    const normalized = steps / pursuitDistanceScale
    // Continuous, bounded, strictly decreasing even for winding paths longer
    // than the map span. Hard clamping would create another zero-signal plateau.
    return normalized <= 1 ? 1 - normalized * 0.5 : 0.5 / normalized
  }
...
function statusBalance(pieces: AIObservedPiece[]) {
  return pieces.reduce((total, piece) => {
    ...
    const friendlyTypes = new Set<string>()
    const hostileTypes = new Set<string>()
    for (const tag of piece.statusTags) {
      if (VALUED_EFFECT_TYPES.has(tag.type)) continue
      if (!tag.sourcePlayerId || explicitTypes.has(tag.type)) continue
      // Like explicit buffs/debuffs, generic labels score once per type and
      // polarity. New instance IDs alone are not additional tactical value.
      const types = samePlayer(tag.sourcePlayerId, piece.ownerPlayerId) ? friendlyTypes : hostileTypes
      types.add(tag.type)
    }
    return total + buffs.size - debuffs.size + friendlyTypes.size - hostileTypes.size
  }, 0)
}

// Public effect contracts, not character/skill scripts. Unknown effects stay
// neutral: arbitrary dynamic code cannot be inferred from a label. These
// bounded estimates are not an extra transition or a promised future outcome.
const VALUED_EFFECT_TYPES = new Set(['elune-protection', 'divine-shield', 'amaterasu-burn'])

function activeEffect(piece: { statusTags?: AIObservedPiece['statusTags'] }, type: string) {
  return (piece.statusTags ?? []).filter(tag => tag.type === type && tag.visible !== false
    && tag.currentUses !== 0 && tag.currentDuration !== 0 && tag.remainingDuration !== 0)
}

function protectionValue(pieces: AIObservedPiece[], observation: AIObservation, playerId: string) {
  const player = observation.players.find(item => samePlayer(item.playerId, playerId))
  // One shared lethal guard is ONE reserve, even with many allies or duplicate
  // tags. A core-health unit offsets its one-time heal without rewarding an
  // unlimited number of labels. Ordinary divine shield is a smaller hit reserve.
  const teamGuard = player && activeEffect(player, 'elune-protection').length > 0 && pieces.length > 0
    ? (pieces.some(piece => piece.isCore) ? 6 : 0.25) : 0
  return teamGuard + pieces.reduce((total, piece) => total
    + (activeEffect(piece, 'divine-shield').length > 0 ? (piece.isCore ? 0.5 : 0.025) : 0), 0)
}

function delayedDamageValue(pieces: AIObservedPiece[]) {
  return pieces.reduce((total, piece) => {
    const burn = Math.max(0, ...activeEffect(piece, 'amaterasu-burn').map(tag => {
      const stacks = Number.isFinite(tag.stacks) ? Math.max(0, tag.stacks ?? 1) : 1
      return stacks * 2
    }))
    // At most one tick, never credit a speculative kill or material removal.
    // A protected target's burn can still consume the protection later; no
    // extra protection credit is awarded until the authority actually removes it.
    const hpAfter = Math.max(Math.min(1, piece.currentHp), piece.currentHp - burn)
    return total + healthValue(piece) - healthValue({ ...piece, currentHp: hpAfter })
  }, 0)
}
...
export function evaluateZeroStageState(
  ...
  // Distance has units of cells, not map AREA. Area-normalization made a step
  // worth only ~425 points on the smoke map, below incidental pressure losses.
  const distanceScale = Math.max(1, observation.map.width + observation.map.height - 2)
  const raw: Record<ZeroStageStaticComponentKey, number> = {
    ...
    protection: protectionValue(own, observation, observation.playerId)
      - observation.players.filter(player => !samePlayer(player.playerId, observation.playerId))
        .reduce((total, player) => total + protectionValue(
          enemy.filter(piece => samePlayer(piece.ownerPlayerId, player.playerId)), observation, player.playerId,
        ), 0),
    delayedDamage: delayedDamageValue(enemy) - delayedDamageValue(own),
