...
const server = createServer((request, response) => {
  ...
  const bases = relative.startsWith('data/') ? [root] : [resolve(root, 'data/pages'), resolve(root, 'public')]
  const file = bases.map(base => ({ base, file: resolve(base, relative) })).find(({ base, file }) =>
