...
for(const file of ['node.exe','server.mjs','Start-Official.cmd','data/pages/official.html','postgres/runtime-manifest.json'])if(!fs.existsSync(path.join(builtRoot,file)))throw new Error('Incomplete server package: '+file)
...
const zip=new AdmZip();zip.addLocalFolder(builtRoot,'Official-Server');zip.addFile('START-HERE.md',guide);zip.writeZip(target)
