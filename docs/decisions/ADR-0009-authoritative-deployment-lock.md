...
> RED-138 取代说明：本文的公开初始站位、重投选择/锁定、独立 45 秒部署时钟与对应超时只适用于 `legacy-reroll-v1` 和缺少 mode 的历史状态；新建对局默认使用 [ADR-0024](./ADR-0024-progressive-reserve-deployment.md) 的私有候选与回合内渐进式部署。房间 CAS、签名边界和通用权威传输条款不受影响。
