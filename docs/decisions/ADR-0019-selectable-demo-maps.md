...
> RED-138 取代说明：本文“确定性部署保持不变”一节中的初始 16 枚出生与重投条款只适用于 `legacy-reroll-v1` 和缺少 mode 的历史状态；受控地图目录、冻结 `Room.mapId` 与地图准入继续有效，新建对局默认部署算法见 [ADR-0022](./ADR-0022-progressive-reserve-deployment.md)。
