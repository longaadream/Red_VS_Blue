...
4. 非空旧 `attachedEffects` 状态在动作入口抛出 `LEGACY_ATTACHED_EFFECT_UNSUPPORTED`；非空旧/custom `initialEffects` 模板在战斗初始化时抛出同一错误码。拒绝守卫不得执行、迁移或静默忽略旧代码。
...
6. 不修改游戏数值、Rule 语义、statusTag 语义、随机算法或公共测试阶段的存档迁移策略。
...
未采用。旧代码载荷被悄悄丢弃会让回放、存档和自定义内容产生不可诊断的行为差异。
...
- 自定义内容若仍携带非空旧字段会得到明确错误，必须改写为 Rule + statusTag。

## 验证方式

- 固定六个代表 Rule + statusTag 场景的结果和最终 state hash。
- 验证非空旧状态在动作执行前拒绝且输入状态不变。
