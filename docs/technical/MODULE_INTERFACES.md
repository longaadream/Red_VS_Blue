...
- 传输：房间 HTTP、WebSocket 和 Relay 使用 `{ state, seed, stateHash, authorityVersion, acceptedClientActionId? }`；玩家命令 applied/duplicate 与 target/option preparation 回显原 `clientActionId`，初始 GET、重连和 timer/system 快照不带 ACK。该字段只属于响应 envelope，Relay 瞬时转发但不写入重连状态 blob；`viewerPlayerId` 只做提交权限校验，不参与站位隐藏。
...
- 动效与反馈（RED-69、RED-99）：`battle-tactical-table.css` 与 `battle-renderer-3d.js` 共享 100/140/240/280ms 的 press/fast/action/result 节奏和同名缓动；renderer 以 `owner:property` 控制器保证同一属性只有一个可中断动效，并以权威状态哈希/序号派生的 `motionEventKey`（缺失时使用确定性展示签名）去重。页面只把单一待确认命令映射为 `BattleViewModel.interaction.pendingPieceId/pendingCommandId`；只有匹配 `acceptedClientActionId`、携带同一 action ID 的明确拒绝、8 秒超时、断线或页面卸载才清理 pending。任意无关 timer/stateUpdate、重复/重连快照或只含诊断字段的变化都不解除 single-flight；`stateHash`/`authorityVersion` 只用于状态去重和排序。合法性、伤害、治疗、状态和死亡仍完全来自前后权威快照。目标格同时进入且不因普通重绘重播，移动从当前可见位置重定向，所有 piece/status/highlight/floater/controller 都在移除或 `dispose()` 时释放。`prefers-reduced-motion` 取消位移、缩放和下沉，仅保留 100–140ms 静态轮廓/淡入淡出及结果文本。
- 待选拒绝恢复（RED-97、RED-69、RED-99 合并语义）：只有携带当前 pending `clientActionId` 的服务端拒绝会结束待确认反馈。若被拒绝的是 pendingTargetSelect 或 pendingOptionSelect，且当前权威快照仍保留对应选择会话，则只结束待确认动效并解除本地提交锁，保留选择 UI 供玩家重试；乱序的旧拒绝不会清除新命令。选择 UI 仅在权威会话结束、回合切换、实体失效、断线或明确取消时清理。
