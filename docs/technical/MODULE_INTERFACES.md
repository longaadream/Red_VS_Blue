...
- 动作历史（RED-166 v2、RED-176）：`battle-presentation-events.ts` 在一次权威命令提交后生成“根动作 → 原子效果”事件链，事件可见性默认是 `public`；仅技能数据中的 `concealTargetInBattleLog` 或权威事件显式声明可将结果设为 `actorOnly`。联网权威流程由 `createPublicBattleTransitionUpdate()` 按具体接收玩家投影；训练/本地权威流程在每次成功命令后调用同一事件生成与查看者投影接口，不使用 URL 参数注入样例事件。对手和观战者拿到的保密结果已折叠为单个 `concealed` 事件，不包含目标类型、选项、坐标、数量或实体 ID；UI 不按动作种类或技能名猜测是否保密。`battle-action-history.js` 只消费当前查看者投影，按根事件去重并保留至多 20 个根动作，右侧 52px 图标栏默认展示最近 5 个，菜单按钮展开主语—谓语—宾语—补语事件链；移动根动作只显示“棋子—移动”，终点只用于棋盘高亮。技能根动作由 `battle-action-identity.js` 优先读取已登记技能展示名；状态和地格效果补语由 `battle-effect-icons.js` 的展示注册表解析中文名称。缺失技能或效果展示元数据时分别记录带事件/类型上下文的错误并显示“未知技能”“未知状态”或“未知地格效果”，不把内部 ID 交给玩家。棋子以头像显示；悬停、聚焦或点击时，历史栏把世界格坐标交给 renderer 的 `setHistoryHighlight()`，由现有 Three.js 相机绘制与棋盘平面平行的临时点和路径，不修改棋盘材质或布局。地格状态是左侧默认紧凑、可展开/关闭的非模态面板，并在复位镜头控件下方保留固定间距，不再触发右侧动作历史收起；技能栏依据左右信息栏安全区停靠。动作历史不发送意图、不执行规则，也不修改战局状态；旧文字 `logOverlay` 仅在显式 `debugBattleLog=1` 时显示，不再是玩家动作历史入口。
