...
- 动作历史（RED-166 v2）：`battle-presentation-events.ts` 在一次权威命令提交后生成“根动作 → 原子效果”事件链，事件可见性默认是 `public`；仅技能数据中的 `concealTargetInBattleLog` 或权威事件显式声明可将结果设为 `actorOnly`。联网权威流程由 `createPublicBattleTransitionUpdate()` 按具体接收玩家投影；训练/本地权威流程在每次成功命令后调用同一事件生成与查看者投影接口，不使用 URL 参数注入样例事件。对手和观战者拿到的保密结果已折叠为单个 `concealed` 事件，不包含目标类型、选项、坐标、数量或实体 ID；UI 不按动作种类或技能名猜测是否保密。`battle-action-history.js` 只消费当前查看者投影，按根事件去重并保留至多 20 个根动作，52px 图标栏默认展示最近 5 个，菜单按钮展开主语—谓语—宾语—补语事件链；移动根动作只显示“棋子—移动”，终点只用于棋盘高亮。技能根动作直接显示权威技能名而不使用图标。棋子以头像显示；悬停、聚焦或点击时，历史栏把世界格坐标交给 renderer 的 `setHistoryHighlight()`，由现有 Three.js 相机绘制与棋盘平面平行的临时点和路径，不修改棋盘材质或布局。地格只显示通用“地格”并高亮，手牌效果只显示通用小卡片。对方手牌以最多五张背面牌的堆叠密度表达，并用数字角标准确显示数量。动作历史不发送意图、不执行规则，也不修改战局状态；旧文字 `logOverlay` 仅在显式 `debugBattleLog=1` 时显示，不再是玩家动作历史入口。
