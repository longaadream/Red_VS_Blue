...
RED-179 补充：动作小剧场的 1× / 2× 速度仍由 `battle-action-vignette.js` 的本地队列控制，但切换入口改为挂载在战场浮层上的常驻按钮；按钮在空闲和播放期间使用同一速度状态，不进入 `BattleState`、存档或服务器消息。
...
RED-186 美漫画风追加：用户最新确认棋盘内允许短促漫画结果字效，替代下述 RED-167 原版“不显示文字”的视觉约束。`battle-action-vignette.js` 在结果阶段从已投影的可见原子事件选取至多一个爆点（移动、召唤或退场）；字效使用既有 `projectCell()` 并限制在视口内，不推测暴击、隐藏结果或缺失目标的位置。空/缺失坐标必须返回无位置，不能转换成 `(0,0)`。既有队列、倍速、略过与减少动态生命周期不变。伤害/治疗只显示原反馈入口提供的实际 −X/+X，不再叠加拟声词；使用字体、描边、爆点底形与棋子侧上方位置。新增 graveyard 中有旧局面可见棋子的确认死亡也补数字，普通移除不推断伤害。玩家 HUD 的 buffSummary 仅投影白名单月神赐福的有效 uses，消耗后隐藏图标。训练添加棋子成功结束放置模式，失败保留重试；历史快照按查看者独立保存，每人最多20根动作；训练在动作前分别保存双方投影，切换回合不清除对方公开历史。动画前态必须保留viewer归属。缺失快照返回当前时必须幂等，避免resize通知导致同步重入；返回工具条必须可点击。

历史来源使用青色八角环，目标使用金色菱形环，连接及弹道使用深色描边箭头；范围覆盖增加稀疏排线。所有几何仍位于棋盘平行平面，地形材质不改写。箭头子描边与排线随其表现对象递归释放；创建和清除表现均唤醒一次按需绘制，减少动态模式也须显示/清除静态结果，不得留下持续 RAF。
...
- Three.js：`battle-renderer-3d.js` 的公共生命周期是 `init(options)`、`update(model)`、`resize()`、`resetView()`、`projectCell()`、`setHistoryHighlight(cells)`、`screenToCell()`、`dispose()`；只负责棋盘内表现。renderer 使用脏标记按需提交帧，静态训练设置页和静态棋盘不保留 RAF；只有状态、镜头、尺寸、纹理或活动动效会唤醒，并在动效清空后再次休眠。20×16 静态地形按材质使用 `InstancedMesh` 批量绘制；范围演出使用与地砖平行的独立覆盖层，不拆散或改写地形实例材质。`setHistoryHighlight()` 保留为兼容接口，将来源点、目标点与连线绘制为棋盘世界坐标中的 Three.js 几何体；空数组清除高亮。RED-186 动作历史先通过 showHistoricalBoard 还原当前大棋盘，再调用该接口绘制历史效果。棋盘状态摘要只在投影变脏时重算。`getPerformanceDiagnostics()` 只读暴露帧数、draw calls、RAF/动效和地形批次用于 QA，不参与规则。`resetView()` 只恢复镜头，不修改战局状态。
- DOM：`battle-ui/battle-dom-ui.js` 只负责 HUD；PR #55 的紧凑生命/负面状态摘要留在棋盘表现层，完整技能与状态继续由 `battle.html` 的右键详情 modal/sheet 和技能栏 `i` 按钮展示，不再创建选中棋子的常驻状态侧栏。`battle-context-layout.js` 只计算棋子菜单锚点、左右信息栏安全区、HUD 上侧安全区和现有 `handCards` 容器的弧形变量，不创建第二份手牌或规则状态。
...
- 坐标与手势（RED-68、RED-162）：`battle-tactical-geometry.js` 只定义固定单轴 42°、35° FOV 的弱透视镜头、低椭圆棋子尺寸、屏幕拖拽到地面坐标的换算与镜头目标夹取；不读取战局状态，也不包含合法性或结算规则。renderer 以 canvas 的 CSS 边界完成指针反投影，优先命中真实地砖几何并以扩展平面覆盖格缝；在 resize/DPR、平移、缩放、镜头复位或棋子移动后发出 `viewport-change`，页面据 `projectCell()` 重算菜单锚点。粗指针的初始缩放遍历透视边缘并保留 1 CSS px 余量，保证所有投影格子的最小轴至少 44 CSS px；10px 累计位移后才从点按切换为拖动。移动态从已选棋子开始的单指拖动优先于镜头平移，renderer 只显示跟手位置并在释放时输出 `drop-piece`；页面仅按 `BattleLegalActions` 返回的高亮集合接收落点，再发送既有 `move` action，权威规则层仍执行最终校验。未命中已选棋子的空白单指拖动继续平移镜头，双指缩放和 `resetView()` 都保留同一 `projectCell()` / `screenToCell()` 合同；`pointercancel`、第二指针、窗口失焦和 `dispose()` 必须取消棋子拖动且不得提交。
- 2.5D 与移动布局（RED-68、RED-166）：棋盘材质、0.30 单位台面厚度、只在前沿可见的立面、棋子轮廓和红蓝阵营环集中在 `battle-tactical-table.css` 与 renderer 表现常量中；棋子最大平面占格约 82% × 72%，厚度 22%，地形仅以有限高度层级表达。战局顶端只保留无独立盒子的双方信息，棋盘舞台不再叠加 CSS 外框，按钮继续使用既有 battle UI，只由响应式层保证触摸尺寸与排布。所有竖屏尺寸都由全屏旋转提示阻止战斗操作；旋转为横屏后恢复棋盘、44px 控件和底部 sheet。目标态隐藏手牌与非必要 HUD，但保留取消与权威目标高亮。该层不得复制游戏规则或引入新的全局状态。
- 动作历史（RED-166、RED-176、RED-186）：权威命令生成根动作与原子效果，经查看者投影后交给 UI；保密结果只显示 concealed，不补造目标。历史栏保留 20 个根动作，按根去重；同根连续同源同类的多目标效果可聚合，原子事件与顺序保留，后续结果用头像代替重复全名。BattlePresentation 为新根保存至多 20 份已投影的行动前 board/pieces/effects/turn 展示副本；训练在权威转换时显式 captureHistory，联网使用转换前模型。首帧、无法证明中间时点的批量追帧没有快照时明确提示不可还原。切换查看者清空缓存并跳过切换当帧捕获。悬停/点击历史记录让当前大棋盘显示保存的局面，renderer.showHistoricalBoard 重建显示对象并保留镜头，setHistoryHighlight 在该历史大棋盘标记目标与位移路径；强制位移以各个被移动棋子的 from/to 配对。历史期间展示模型仍接收最新状态，棋盘指令及页面行动输入被拦截；返回当前或 Esc 恢复最新收到的战局。清除历史活动 ID 必须先于恢复 renderer，防止镜头 resize 同步重入。没有第二张小棋盘，没有修改权威状态、哈希、规则或存档；缓存只属于表现层生命周期。事件 history 的相关实体/坐标元数据仍用于路径说明，不用于伪造完整棋盘。旧 logOverlay 仍仅 debugBattleLog=1 可见。
- 动作小剧场（RED-167）：`battle-action-vignette.js` 与动作历史消费同一批查看者已投影的权威 `presentationEvents`，按 `rootEventId` 串行播放且用 `eventId` 去重。技能根动作通过 `battle-action-identity.js` 解析施法棋子头像与权威技能名称，头像缺失时以棋子名首字兜底；顶部状态条与动作历史均显示“棋子头像 + 技能名称”，棋盘内不显示文字或技能图标。训练模式通过页面实际加载的 `game-engine.js` 调用共享 `projectBattlePresentationEvents()` 与 `projectBattlePresentationEventsForViewer()`，不再维护独立投影器副本。根事件可携带只读 `presentation` 提示：`cue`、玩家选择的 `selectedCell`、有序 `pathCells`、实际 `endPoint/endReason`、`collisions` 与 `areaCells`；其中弹射物终点必须来自结算后的实际轨迹，不能用点击格替代。表现声明不进入 `BattleState`、哈希、patch 或持久化，也不参与命中与伤害规则。小剧场由 `BattlePresentation` 统一 mount/update/resize/dispose，普通节奏 1100ms、2× 模式保持事件顺序、略过目标为 60ms 收束并吞掉本次战场输入；轮到观看者行动时直接收束，重连首帧不回放历史积压，`prefers-reduced-motion` 使用静态结果帧。棋盘内的指向/弹射表现由 renderer 的 `showPresentationPath()` 在固定棋盘平面高度创建世界空间 ribbon 和中心虚线；范围效果通过 `showPresentationAreaFlash()` 创建与真实地砖表面平行的独立 emissive 覆盖层，以兼容批量实例地形并继承棋盘角度、透视与遮挡。护盾拦截只展示静态结果；召唤（含复活）使用独立入场反馈，pending 角色使用阵营环和本体高亮。变身专属动画延期。
- 玩家效果与关键词（RED-186）：BattleDomUI.playerEffectRegistry 为明确指定的玩家效果白名单，后台规则默认不显示。玩家资源牌旁直接显示漫画 SVG 图标，悬停/聚焦给出紧邻简述；艾露恩守护依据可见状态出现、消耗后移除，古尔丹裂魂依据玩家规则出现。没有全队规则列表或侧拉栏。动能、万花筒、灵压的正文直接精简，天照关键词接入天照之炎/加具土命/因陀罗之矢；面板只显示简洁说明，不再放长篇规则细则。
...
- `lib/game/turn-timer.ts`：定义可注入 `AuthoritativeRuleClock`、90/120/150/180/210 秒成长时长、40 秒快速回合、30 秒回合外 pending 响应和最终 15 秒烧绳、当前输入所有者、活动回合玩家、同回合各输入 owner 的剩余预算/烧绳状态、玩家独立连续无操作次数和只读投影。
...
- `scheduleRoomBattleTimeout()`：仅在显式 `RVB_TURN_TIMER_ENABLED=1` 时启用 deadline，并按烧绳阈值和当前成长回合期限安排下一次唤醒；Colyseus 路径注入 `Room.clock.setTimeout()`，但唤醒后的系统命令仍进入同一个房间 FIFO。渐进候选与部署共用该期限，超时自动完成必要部署并直接继续既有超时流程。每次提交清除旧任务并按最新 deadline 安排唯一下一次唤醒；未设置或 `0` 时清除既有任务、隐藏计时投影，且晚到玩家动作不会被替换成 timeout。烧绳/超时各只提交一次；超时进入 bot action phase 时调用 bot-turn 回调。
...
## RED-158 Windows Colyseus + PostgreSQL 接口
