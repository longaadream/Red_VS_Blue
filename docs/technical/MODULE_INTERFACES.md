...
- 动作历史（RED-166）：`battle-action-history.js` 只消费展示模型中的权威 `presentationEvents`，按根事件折叠子事件、去重并保留至多 20 个根动作，默认渲染最近 5 个图标。技能根动作通过 `battle-action-identity.js` 显示施法棋子头像，并在悬停/聚焦和无障碍名称中显示具体技能名；头像缺失时使用棋子名首字兜底。它由 `BattlePresentation` 统一 mount/update/resize/dispose，使用 renderer 的 `projectCell()` 在既有 `floatLayer` 做短时来源—目标—路径高亮；不发送意图或游戏命令，也不修改战局状态。右侧浮窗、状态摘要、目标模式、系统弹窗、窄宽或低高横屏会令 52px dock 收为单个 44px 日志按钮，完整文字日志继续作为二级入口。
- 动作小剧场（RED-167）：`battle-action-vignette.js` 与动作历史消费同一批权威 `presentationEvents`，按 `rootEventId` 串行播放且用 `eventId` 去重。`battle-view-model.js` 只把技能字典投影为不含执行代码的 `skillSummariesById`，`battle-action-identity.js` 再以 `sourcePieceId/skillId` 统一解析施法者头像和技能名。联网链路直接消费 authority transition 携带的事件；本地训练链路通过浏览器包 `battle-presentation-events.js` 调用同源 `projectBattlePresentationEvents()`，以已提交动作和结算前后状态生成相同结构，不解析日志或自行重算规则。该浏览器包的更新命令为 `npx esbuild lib/game/battle-presentation-events.ts --bundle --platform=browser --format=iife --global-name=BattlePresentationEvents --minify --banner:js="/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-unused-expressions -- generated browser bundle */" --outfile=data/pages/js/battle-ui/battle-presentation-events.js`；页面合同测试用同参数在内存重建并逐字节比较，源与包漂移时必须失败。根事件可携带只读 `presentation` 提示：`cue`、玩家选择的 `selectedCell`、有序 `pathCells`、实际 `endPoint/endReason`、`collisions` 与 `areaCells`；其中弹射物终点必须来自结算后的实际轨迹，不能用点击格替代。投影器以表现层专用的静态声明区分首次碰撞、穿过棋子与选择目标三种路线；声明不进入 `BattleState`、哈希、patch 或持久化，也不参与命中与伤害规则。小剧场由 `BattlePresentation` 统一 mount/update/resize/dispose，普通节奏 1100ms、2× 模式保持事件顺序、略过目标为 60ms 收束并吞掉本次战场输入（验收上限 100ms），轮到观看者行动时直接收束；重连首帧只登记既有根事件，不回放历史积压。`prefers-reduced-motion` 使用 120ms 静态结果帧。棋盘内的指向/弹射表现由 renderer 的 `showPresentationPath()` 在固定棋盘平面高度创建世界空间 ribbon 和中心虚线，高地只遮挡而不能抬歪轨迹；DOM 不创建轨迹、动作文字、图标或结算徽章，施法者头像与技能名只显示在棋盘外的顶部状态条和动作历史。范围效果由 renderer 通过 `showPresentationAreaFlash()` 临时克隆权威地砖的原材质并提高橙色 emissive，真实 tile mesh 的 geometry/position/scale 不变，收束时恢复原材质；因此效果直接继承棋盘本身的角度、透视、厚度和遮挡，不创建屏幕空间 DOM 光晕或额外覆盖面。护盾拦截只展示静态结果；召唤（含复活）使用 renderer 独立入场反馈，pending 角色使用阵营环和本体高亮。变身专属动画按产品决定延期。全部计时器、DOM、监听器和临时外观控制器必须在结束或 `dispose()` 时清理。
