...
训练营直接使用 `applyBattleAction`，该入口在调用方未提供 EffectChain 时建立临时 detached 动作链，使同一次技能中的各段伤害共享死亡记录；已有 `runBattleAction` 链则复用。动作返回或抛错后清理临时链及其克隆状态绑定，记录不会跨动作延续。`validateSkillActionByDryRun` 当前只进行声明式目标和基本条件检查，不执行技能伤害；此次不改变其语义。
