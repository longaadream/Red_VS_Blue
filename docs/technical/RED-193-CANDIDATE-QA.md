...
## RC2：Colyseus 观战补充（2026-09-08）

基线已刷新为 `main@b8a26a90fc591b61fef9fa33db4e679fb75eb33b`，使用 merge 保留 PR 历史。新技能流程、状态生命周期和练习模式保留；JSON 冲突按三侧属性合并，只将 RED-193 的团队关系/实例来源判断移植到 main 新实现。生成的 Windows 引擎与练习引擎重新构建。

- 联机回归：`npm.cmd run test:multiplayer`，17 文件 / 44 项通过。含1v1和2v2的8观众、玩家席位独立、私密邀请码、公开加入、只读命令/回执边界、两次原生重连后关闭实际 socket、异步快照读取期间撤权、终局隐藏信息、战报匿名401/旁观者403、8名掉线观众撤销预约后立即补满、主机重建默认关闭观战。
- HTTP完整战报已改为参赛身份一次性签名、公钥与持久化席位匹配；转发仅允许固定认证头，不向公网节点增加裁定职责。
- 额外隐私复查：观众快照和battleProfile均不带真实随机种子，删除动态手牌定义；影分身以公开展示属性/位置身份投影，内部本体引用不再泄露秘密选项。互换本体/分身位置投影一致，本体与分身分别死亡时的实际界面模型可见性回归通过；10项公开投影测试和3项真实Colyseus观战测试通过。参赛者状态和裁定不受该观众展示处理影响。
- 合并后 TypeScript、全仓 ESLint、编码检查及 Windows Colyseus边界检查通过。测试夹具补齐新页面模式标志与类型，不更新冻结快照。
- 另行串行验证页面协议、卡牌元数据、公开投影、客户端重连、四席flow查询、新技能规则、AI环境与练习模式：15文件 / 159项通过（`rc2-adjacent.txt`）。
- 首次Windows包实测发现main规则执行版本已升级v2，Electron仍硬编码v1，导致本机权威启动后身份校验失败；已同步桌面声明/校验并用跨模块合同测试防止再漂移，旧失败日志保留，修复后须重新构建和冒烟。
- 大范围首次运行：游戏/练习1288通过、15失败。部分为新页面模式缺失的VM夹具和并行资源争用，已修正并另跑相邻检查；剩余历史失败涉及AI语义清单、静态触发器/技能计数、Sonic文字、targeting冻结hash及未生成Android bundle。不能称全仓规则测试通过。
- 规则与网络测试应串行执行：共同使用`resource-pack/active.json`，同时运行曾触发 Windows rename EPERM。`rc2-network2.txt`是隔离执行后的通过结果，不把失败运行计为成功。
- 当前容量复测：100条本机转发连接、2000次1KB往返、3000次目录读取；P50约74ms、P95约126ms，RSS约232MiB。仍不是100人完整对局或中国公网验收。

原始本地证据放在`dist/multiplayer-qa/rc2-*.txt`。Windows候选验证入口为`smoke:multiplayer:windows`：实际点击大厅观战，核实脱敏手牌、蓝红面板、原参赛恢复记录、终局只读界面，并保留既有安装、内嵌数据库、恢复和退出检查。构建与冒烟的最终结果应以本次候选日志、PR验收记录及`release.json`对应提交为准；源码检查不代替实际包验证。

RC2产物使用`package:multiplayer -- RED-193-rc2`，保留RC1。真实frp、异地Windows、多运营商和分批大规模实战仍须组织者验证，完整教程冒烟仍是独立验收项。观战操作、主机重启后重新授权及回退见试玩指南。
