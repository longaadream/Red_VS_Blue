...
The RED-72 regression suite covers fixed minimal states for blocked `beforeMove`, trigger exceptions, action rollback, event consumer continuation, and attached-effect dispatch. The required validation order is focused regression, affected-module tests, `npm test`, and `npm run build:game-engine`.
