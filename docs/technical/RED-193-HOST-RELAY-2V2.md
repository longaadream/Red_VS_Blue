...
用户确认继续使用 Colyseus：实时、每房最多8名独立观众；仅公开战场，隐藏全部手牌/秘密选择；房主开关、公开房直接观看、私密房需12位邀请码。沿用原生 joinById / allowReconnection / onReconnect 和 battleSnapshot，不新增游戏 WebSocket 协议。观众不能调用参赛 RPC、receipt 查询或游戏命令。完整战报 HTTP 读取使用一次性身份签名，并匹配持久化参赛公钥。终局仍隐藏观众的 trace/replay。

关闭观战须立即撤权、终止当前连接并释放掉线预约；异步读取快照后再次核实资格。主机进程恢复时默认关闭观战，不从旧 gameplay checkpoint 自动恢复开放权限。蓝红双方固定展示，观战不改参赛恢复记录。

2026-09-08 QA前刷新发现 main 已推进至 `b8a26a90fc591b61fef9fa33db4e679fb75eb33b`，保留发布分支历史，合并后重新执行检查；先前结果不代替新基线结果。候选编号 RC2，RC1 ZIP 保留用于回退。
