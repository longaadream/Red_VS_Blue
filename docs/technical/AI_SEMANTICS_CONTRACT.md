...
覆盖矩阵由 `npm test -- tests/game/ai-semantics.test.ts` 调用的 `scripts/audit-ai-semantics.mjs` 输出：automatic、metadata-required、evaluator-required、unsupported 及具体 ID。`evil-explosion` 是已知缺失资源，明确 unsupported，绝不静默推断。
