...
在主线 `e9b6918080010f30f5b2fe5f535548f80c90a257` 上修复配置适用范围、源码类型和测试构造器；suppression 从 740 项降至 615 项，没有增加任何文件/规则桶的豁免数量。

- Next 插件规则只作用于插件实际注册的文件类型，避免新增 `.cjs` 工具脚本令整次 ESLint 配置加载失败；CommonJS 文件允许其原生 `require` 语法。
- `data/pages/js/colyseus-sdk.js` 是第三方浏览器发行包，精确排除该文件，继续检查业务源码。
- 独立 HTML 页面脚本允许直接导航到页面文件；Next 页面仍检查对应导航规则。
- 为本次整理的四个浏览器脚本增加 `no-undef`，并保留错误处理仍使用的 catch 参数。
- 清理无用声明和多余 `any`，补充技能上下文、语义输入、测试夹具及数据库 JSON 边界类型。

新增 `tests/eslint-config.test.ts` 验证上述边界及源码/测试的 `no-explicit-any` 门禁。`.github/workflows/lint.yml` 在主线 push 和面向主线的 PR 上运行全仓 lint 与配置回归测试；本地候选合入后才会生效。

本次恢复的是全仓 lint 零错误、零警告，并减少历史技术债；仍有 615 项既有豁免，不能将通过门禁理解为所有历史违规已经归零。修复授权、独立审查、完整测试对照与构建限制见 [本地修复记录](../qa/RED-88-eslint-repair.md)。
