...
零阶段 AI 的正式 agent/profile ID 为 `rvb-ai-zimse-v1`。它是无需训练、确定且可解释的 player-level 基线，不替代 RED-86 Beam Search，不加入训练联赛，不修改规则、数值、随机或存档。经用户批准，它通过现有 Colyseus 权威入口作为 PvE 普通难度提供给玩家，具体边界见下文。
...
主菜单“开始冒险”提供简单（`easy`，sample-v1；代码中的旧 ID 为 `simple-v1`）和普通
（`normal`，`rvb-ai-zimse-v1`）。默认简单。页面将 `difficulty` 经 `ws-client.js` 传给当前
Colyseus BattleRoom；HTTP/legacy WS 创建入口使用相同白名单。缺省及旧房间保持简单，未知值拒绝，
不接受客户端自定义权重或任意 agent。Bot 的 `botDifficulty` 随现有 players 元数据保存，无 schema 迁移。

Colyseus 创建 PvE 时加入蓝座 Bot，真人占红座；沿用 `lockDefaultBotRosterInStore` 锁定正式阵容。
Bot 座位不能由客户端登录。普通每次从当前权威状态调用零阶段估价，只将所选原始动作通过现有
`dispatchRoomBattleAction` 提交，不能用简单 AI 重选技能目标；提交后广播原有 Transition，再重算。
部署/pending 同样只推进一步；思考通过既有 room authority pause 接口排除计时。简单保留旧批次策略，
每条动作提交前重新校验。候选全集不因 UI、延迟或难度裁剪。

普通动作计数只在 `applied` 后增加，按 root seed、回合号与当前回合玩家隔离；同一存活 Room 内
pending 人类响应、重新调度不会重置。进程恢复不承诺还原瞬态调度计数（不改变现有恢复协议）。
Bot 错误带 room/turn/计数记录并停止该次任务，原权威 timer 仍能推进；不伪造成功动作。
全量枚举仍同步运行，复杂局面可能阻塞同进程请求；目前不能保证每局 10 分钟或 20/40 回合胜率。
...
v7 将渐进部署仍视为一个普通单步策略：每个“候选棋子 × 权威合法落点”只执行一次部署 transition，然后估价部署后的公开局面。若新棋子带有公开 `deployment-first-move-free` 标签，`enemyProximity` 与 `futureAttackPotential` 将接敌距离减去该棋子免费移动范围的 50%；前者使用可行走最短路，后者使用曼哈顿距离。未兑现机动只按半价计入静态潜力，不枚举或模拟第二个动作。实际有效推进后由真实位置接替潜力，固定局面中完整免费推进的追敌分严格更高；最终选择仍同时受生命、安全和其他分项影响，不保证每个局面都移动。
