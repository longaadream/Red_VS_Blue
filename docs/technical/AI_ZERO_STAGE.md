...
| `enemyProximity` | 80,000 | 常态按每个棋子的最近敌核追击；仅在己方核心数占优的清场阶段混入一对一匹配，避免优势兵力全围住最近目标而漏追远处残敌 |
...
位置分项不读取角色、技能或地图 ID。`strategicPosition` 使用地图宽高得到几何中心和目标压迫，但 v5 把它降为辅助项。独立 `enemyProximity` 使用公开可行走格 BFS：常态取每个己方棋子的最近敌核距离；仅在己方核心数多于敌方的清场阶段，按敌我数量比混入双方核心的最大接近度一对一匹配，让一部分优势兵力追击远处残敌而不改变前期接敌策略。敌方没有存活核心时退化为普通敌棋。最短路距离按公开可行走格数量归一化，使追击保持连续但不会压过真实伤害和生存；该分项不减去敌方镜像距离，因此主动推进信号不会抵消。未来攻击按每个目标选择最强己方来源，仅使用公开攻击、防御、生命、移动范围和几何距离；核心目标权重高于非核心实体。支援是通用汇合能力，不猜测某个角色是否具有治疗技能；移动空间通过公开可行走格与存活棋子占位做有界 BFS；地形只读取格子公开属性。

核心接敌最短路场按地图尺寸和公开可行走格键控，跨等价候选复用，最多保留 8 个最近使用的地图拓扑。缓存不包含棋子占位、手牌、玩家私有字段或候选结果；地图 walkable 变化时得到新的键。

一对一匹配的动态规划只在己方核心数确实多于存活敌核时执行；开局、势均力敌、劣势及敌核已清空的终局直接返回最近目标结果。该短路不删除或跳过任何合法动作，只避免计算最终权重为零的估价子项。
