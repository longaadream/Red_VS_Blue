...
`data/pages/js/battle-audio.js` 另根据已投影的战斗事件提供伤害、治疗、移动、传送的本地合成音。此自动映射不扩大上述作者 cue 接口。音效只消费实际结果：零伤害/零治疗、取消及失败事件不响；强化与削弱暂不映射声音。

`battle-presentation.js` 在动作表现阶段调用 `playEvents`：移动在 path、其他效果在 result、减少动画时在 static 播放，同一演出片段去重。settle（略过）及历史查看不补响；同批次同类多目标效果合并成一声，不同批次的多段效果仍各自播放。重连恢复期间及恢复后的首快照重建演出基线，不补播断线期间的动作和声音。音量默认45%，主菜单可调节并保存，首次用户操作前不自动发声。静音、进入后台及销毁播放器会停止已经安排的短音效；恢复前台不重放旧声音。最多12个合成声部，无音频依赖或网络下载。
...
伤害/治疗按实际 amount 的 1–3、4–7、8+ 分三档；移动按投影结果端点的视觉距离 1–2、3–4、5+ 分档，同批同类采用最大值而非相加。传送仍使用独立音色。BattleImpact 仅对重伤害在表现阶段触发 3px / 180ms 震动，独立 translate 不修改棋盘缩放。主菜单可关闭，尊重减少动态效果；用户输入、跳过、历史、恢复、后台及销毁都会终止震动。声音和震动均不改变权威规则与输入提交。

按钮反馈由 button-audio.js 委托监听真实 click，使用 BattleAudio 的 click 音色，复用音量/静音。禁用、inert、data-no-click-sound 和 data-kind 试听控件排除；不监听棋盘落点或拦截操作，页面历史恢复时重新建立播放器。
