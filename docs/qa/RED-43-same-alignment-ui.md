...
2026-08-15，人工验收者按上述入口完成 Alice/Bob 四个视角检查并确认通过：

- 固定席位映射为 Alice=红、Bob=蓝；两人使用相同内容阵营时，敌我集合仍随 `playerId` 正确反转。
- 光/光场景的敌方目标为 UI 8 / 规则 8；天翔龙闪仅允许选择 Bob 棋子，合法目标实际结算 8 点真实伤害。
- 暗/暗场景的友方目标为 UI 8 / 规则 8；邪能赐福仅允许选择 Alice 棋子，合法目标实际结算攻击 +2、受到 2 点真实伤害。
- 两个场景的 Bob 视角均保留 Alice 视角产生的结算状态，并正确交换我方/对方集合。

非阻塞体验观察：首次查看时 Alice/Bob 与红/蓝席位的映射不够直观；该观察不影响 RED-43 合同验收。
