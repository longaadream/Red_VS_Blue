...
| 验收项 | 自动证据 | 结果 |
| --- | --- | --- |
| 不同阵容、规则与 seed 的两个房间各 100 次交错 Transition | 与各自单房串行的最终 state hash、RNG cursor、pending 和 rule limits 全量比较 | 通过 |
| 异常不跨房间传播 | room-a 故障规则抛出后，room-b 正常提交且计数独立 | 通过 |
| pending、限制计数和编译缓存隔离 | 两房分别检查 pending、rule uses 与不同 DynamicCodeRuntime 实例 | 通过 |
...
- 聚焦隔离：`npm.cmd test -- tests/game/room-runtime-isolation.test.ts` — 4/4 通过。
- 确定性运行时：`npm.cmd test -- tests/game/deterministic-runtime.test.ts` — 13/13 通过。
- 关键兼容回归：`npm.cmd test -- tests/game/turn.test.ts tests/game/turn-timer-room.test.ts tests/game/battle-authority-v2.test.ts tests/game/room-runtime-isolation.test.ts` — 72/72 通过。
- 异步权威性能：`npm.cmd test -- tests/game/battle-authority-async-dispatch.test.ts` — 1/1 通过。
- 受影响模块：9 个测试文件、110/110 测试通过。
...
- 游戏层全回归（排除需预生成 Android bundle 的冻结哈希文件）：91 个测试文件、780/780 通过。
