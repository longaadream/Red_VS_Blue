...
- 1280×720 范围闪烁：`output/playwright/RED-167-area-flash.png`。演出层处于 `is-cue-area`，只渲染 3 个权威 `areaCells` 的闪烁轮廓。
- 844×390、`prefers-reduced-motion: reduce`：`output/playwright/RED-167-reduced-motion-mobile-landscape.png`。演出阶段为 `static`，路径元素计算样式为 `display:none`，速度按钮为 44×44px。
