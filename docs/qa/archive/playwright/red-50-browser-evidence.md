...
- 桌面截图：[历史截图：red-50-desktop-1280x720-status.png](https://github.com/longaadream/Red_VS_Blue/blob/a7849ce9eb3fc9e2fbc000afdd5f770b6f57e37a/output/playwright/red-50-desktop-1280x720-status.png)
- 最低横屏截图：[历史截图：red-50-mobile-844x390-status.png](https://github.com/longaadream/Red_VS_Blue/blob/a7849ce9eb3fc9e2fbc000afdd5f770b6f57e37a/output/playwright/red-50-mobile-844x390-status.png)
- 推荐横屏截图：[历史截图：red-50-mobile-932x430-status.png](https://github.com/longaadream/Red_VS_Blue/blob/a7849ce9eb3fc9e2fbc000afdd5f770b6f57e37a/output/playwright/red-50-mobile-932x430-status.png)
...
- 桌面截图：[历史截图：red-50-desktop-1280x720-target.png](https://github.com/longaadream/Red_VS_Blue/blob/a7849ce9eb3fc9e2fbc000afdd5f770b6f57e37a/output/playwright/red-50-desktop-1280x720-target.png)
- 最低横屏截图：[历史截图：red-50-mobile-844x390-target.png](https://github.com/longaadream/Red_VS_Blue/blob/a7849ce9eb3fc9e2fbc000afdd5f770b6f57e37a/output/playwright/red-50-mobile-844x390-target.png)
- 推荐横屏截图：[历史截图：red-50-mobile-932x430-target.png](https://github.com/longaadream/Red_VS_Blue/blob/a7849ce9eb3fc9e2fbc000afdd5f770b6f57e37a/output/playwright/red-50-mobile-932x430-target.png)
...
- 390×844 中只显示“请旋转设备”，遮罩覆盖整个战斗界面并拦截交互；截图：[历史截图：red-50-mobile-390x844-rotate.png](https://github.com/longaadream/Red_VS_Blue/blob/a7849ce9eb3fc9e2fbc000afdd5f770b6f57e37a/output/playwright/red-50-mobile-390x844-rotate.png)。
...
- 最终合入基线：`origin/main@12e1327`（继续包含 RED-49，并新增 RED-67 玩家阵营对齐、RED-72 战斗触发原子性、RED-29 确定性部署等主线改动）。
...
- 与 `12e1327` 的唯一文本冲突位于 `applyServerState()`：保留主线 `playerAlignments` 快照同步，并在其后继续调用 RED-50 的 `reconcileBattleInteractionState()`。
- 冲突直接回归：6 个文件、52 项测试通过；全量并行 Vitest 为 31/36 个文件、300/308 项通过，8 项失败均为 5 秒超时且没有断言差异。
- 将 5 个超时文件改为单线程复跑后，5 个文件、36 项全部通过。TypeScript、543 个文本文件的编码检查、相关文件 ESLint 和 `diff --check` 均通过。
- `tests/game/targeting.test.ts` 单独 ESLint 仍报告 14 个既有 `no-explicit-any`；逐行 blame 均来自已合入主线的 RED-59 提交，RED-50 在该文件只修改两条目标提交正则断言。

## Console

- 在 `844 × 390` 真实浏览器中同时验证“暗 先手”阵营标签、纯特殊状态栏、技能可用性、进入目标模式与取消恢复；RED-50 交互期间没有新增 Console error，也没有 `TypeError` 或 `ReferenceError`。
