...
> RED-138 兼容说明：本记录中的“45 秒部署锁定”只验证 `legacy-reroll-v1`。新建 `progressive-reserve-v1` 对局没有独立部署锁定时钟；候选、落位与免费移动使用当前成长回合的同一期限。
...
1. 验证 legacy 时，用两个浏览器会话加入显式 `legacy-reroll-v1` 房间，完成阵容和 45 秒部署锁定；验证新建渐进对局时，改为确认双方随机先锋完成后进入当前回合的私有候选，并且不存在独立部署锁定倒计时。
