...
| 指标 | P50 | P95 | P99 | 最大值 |
| --- | ---: | ---: | ---: | ---: |
| v2 完整 dispatch（内存 journal） | 11.849 ms | 15.816 ms | 16.597 ms | 16.654 ms |
| v2 持久化模拟 | 0.150 ms | 0.467 ms | 0.650 ms | 0.678 ms |
...
- 前 10 次 dispatch 中位数为 11.537 ms，后 10 次为 12.059 ms，没有随历史长度增长。
- 首条 v2 Transition 的载荷包含版本 0 迁移基准检查点；换回合、固定间隔和终局检查点解释了 P99 的 32,175 B，普通动作仍约 10 KB。
- 规则文件缓存与同步日志门控后的旧完整快照后备路径，在同一次最终运行中 dispatch 为 P50 35.445 ms、P95 61.414 ms、P99 68.628 ms；它是未设置或 `RVB_BATTLE_AUTHORITY_V2=0` 时的默认回退，不是候选正常链路。
