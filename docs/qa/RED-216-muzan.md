...
- 删除无惨除「定身」以外的显式关键词，血肉再生正文去掉「被动：」。
- 无惨之躯补「可穿过友军」，实际目标筛选、预检和提交路径均支持穿友军。先建立相邻友军阻挡导致无法选择远处落点的失败测试，再修正配置；友军不受伤、不被定身，落点仍要求空格且不可穿墙。
- 未见死亡结晶的验收局使用了训练工具放置的非核心无惨。既有规则只让核心棋子掉落，这不是寄生吞掉结晶。新增回归确认核心无惨在传送落点保留一个真实、可见的结晶地格效果，非核心成功寄生后不掉结晶。重新验收时应在开局设置选择核心无惨。
- 本轮 5 文件角色回归 **41/41 通过**，typecheck、相关 ESLint 与独立复核通过。实际浏览器新开核心无惨训练局，面板已显示「核心棋子」、去除重复被动前缀、仅「定身」关键词及「可穿过友军」描述。当前修改仅涉及运行时加载的技能 JSON，无需重建引擎；旧对局应重新开局以加载新技能定义。
