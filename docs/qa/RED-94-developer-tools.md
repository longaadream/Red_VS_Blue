...
- 独立只读回放页复用正式 `BattleViewModel` 和 `BattleRenderer3D`；棋盘覆盖整个视口，帧摘要、时间轴、逐步/播放/速度/视角控制和可折叠检查器以浮窗叠加。
- 检查器按命令、棋子、事件、变化和完整性分组；棋子页展示技能名称/描述、当前与基础冷却、剩余次数、充能和解锁状态。
...
- 每个 `frame` 保存一条成功命令和该命令后的 `postState`。该帧的命令前状态就是上一检查点，因此不会重复保存相邻帧相同的大状态。地图未变化时帧以 `inheritsMap: true` 继承上一检查点地图；导入器先物化完整状态再验证和显示。
- `preStateHash/postStateHash` 是权威规则状态 hash；`preCheckpointHash/postCheckpointHash` 是导出检查点的 SHA-256 链。导入器同时验证两条链。
- 每帧保存 `events` 和随机流游标；累积 `actions` 不重复进入检查点，检查器无需猜测状态变化原因。
- `content` 只保存回放显示所需的棋子/技能名称、描述、类型、基础冷却/充能/消耗和安全本地图片标识，不保存技能执行代码。每帧棋子状态保存当前冷却、充能、解锁与剩余次数；缺失内容时显示稳定 ID。
...
npm.cmd run lint
npm.cmd test -- --maxWorkers=1 --testTimeout=120000
npm.cmd run build
...
- RED-94 聚焦测试：7 个文件、52 项断言通过。
- 全量 Vitest：`--maxWorkers=1 --testTimeout=120000` 下 82 个文件、649 项断言通过。默认并发首次运行有 5 项慢测试超时；对应 4 个文件单独复跑全部通过，串行全量随后全部通过。
...
- 1440×900 下棋盘覆盖完整视口，底部控制栏与右侧五标签检查器均为浮窗；检查器可以收起。
- 点击 `(9,9)` 的“观者”后，技能页显示“红蓝形态 / 潦草急救”、当前冷却 `0（就绪）`、基础冷却 `0 / 4 回合`、剩余次数 `无限 / 1`、充能与解锁状态。
- 检查器展示终局、事件、随机流以及两条 hash 链；未知事件使用通用显示。
...
- 相邻帧不重复保存 `preState`；不变地图继承且累计 actions 不重复写入，但物化后的检查点 hash 和权威 hash 链连续。
...
- 回放页加载正式战场展示模块；棋盘占满视口，控制栏和可折叠标签检查器为浮窗。
...
1. 点击“打开可视化回放”，预期进入独立页面，棋盘覆盖整个视口并停在第 0 帧初始状态；帧摘要、底部控制栏和右侧检查器是浮窗。
...
6. 点击棋子，预期检查器自动打开并切到“棋子”；显示名称、实例 ID、归属、位置、生命、基础属性，以及每个技能的记录名称/描述、当前冷却、基础冷却、剩余次数、充能和解锁状态。
7. 切换命令/事件/变化/完整性标签并关闭、重新打开检查器；未知事件应以类型和安全字段通用展示，浮窗不能改变 Trace 或遮断棋盘全屏使用。
...
- 单文件上限 32 MiB。固定 seed 的 4 命令样本为 135,349 字节（旧实现带缩进且重复地图时为 588,507 字节）；固定部分 67,746 字节，样本平均每帧约 16,900 字节，按此估算约 1,981 个简单命令帧。复杂选择会话、卡牌和扩展状态会降低容量；超长比赛仍可能需要后续分块/流式格式。
