...
- 动作栏展开为主语—谓语—宾语—补语结构；悬停、聚焦或点击仅使用现有 `projectCell()` 叠加临时高亮，不修改棋盘材质、布局、规则状态或 hash。
- 本机正式 QA 路由先从 `data/pages/images` 加载战斗页图标，再回退旧 `public` 图标；真实棋盘验收不再依赖脱离战场的纯 UI 夹具。

## 自动验证

```text
npm.cmd test -- tests/game/battle-presentation-events.test.ts tests/ui/battle-action-history.test.ts tests/ui/battle-effect-icons.test.ts tests/game/battle-page-contract.test.ts
...
node --check data/pages/js/battle-ui/battle-effect-icons.js
node --check data/pages/js/battle-ui/battle-view-model.js
passed
...
隐私回归同时断言：普通选择没有 `actorOnly` 标记且公开显示；影分身的选项阶段和地格阶段只对行动玩家显示；对手/观战者的序列化载荷不包含私密文本、坐标或标识符；实际传输边界和非本人手牌占位数据不泄漏原卡牌 ID。UI 集成测试还断言动作历史获得的是当前战场 renderer 的真实 `projectCell(x, y, elevation)` 委托，而非另一套坐标换算。
...
3. 连续产生 20 个根动作，展开/收起动作栏，悬停棋子与地格记录；确认棋盘外观不变，只有临时来源/目标高亮。
4. 在 1280×720、844×390、390×844 和 16 棋子局面下检查右侧浮层、技能菜单、状态详情和系统弹窗。
5. 以 `?debugBattleLog=1` 验证旧日志仅供调试；去掉参数后入口必须不可见且不可聚焦。
